...
ax = plt.subplot(131)
masked_array = np.ma.array(np.array(vel_narrow)-vsys,mask=np.isnan(vel_narrow))
im = plt.imshow(masked_array,cmap='jet',origin='lower',extent=[-6.25,6.25,-5.65,5.65],vmax=400.)
...
im = plt.imshow(masked_array,cmap='jet',origin='lower',extent=[-6.25,6.25,-5.65,5.65],vmin=0.)
...
im = plt.imshow(masked_array,cmap='jet',origin='lower',extent=[-6.25,6.25,-5.65,5.65])
...
ax1.set_yticklabels([])
plt.colorbar(cax=cax1)
ax1.set_title(r'Flux$_{[NII]6584}$')
...
ax2.set_yticklabels([])
plt.colorbar(cax=cax)
ax2.set_title(r'Flux$_{[NII]6548}$')
...
ax3.set_ylabel('$\Delta \delta$ (arcsec)')
plt.colorbar(cax=cax)
...
ax6 = plt.subplot(236)
masked_array = np.ma.array(flux_OI1,mask=np.isnan(flux_OI1))
im = plt.imshow(masked_array,cmap='jet',origin='lower',extent=[-6.25,6.25,-5.65,5.65])
...
ax = plt.subplot(131)
masked_array = np.ma.array(np.array(vel_second)-vsys,mask=np.isnan(vel_second))
im = plt.imshow(masked_array,cmap='jet',origin='lower',extent=[-6.25,6.25,-5.65,5.65])
divider = make_axes_locatable(ax)
cax = divider.append_axes("right", size="5%", pad=0.05)
plt.colorbar(cax=cax)
...
ax = plt.subplot(121)
masked_array = np.ma.array(np.array(vel_broad)-vsys,mask=np.isnan(vel_broad))
im = plt.imshow(masked_array,cmap='jet',origin='lower',extent=[-6.25,6.25,-5.65,5.65],vmin=-400)
divider = make_axes_locatable(ax)
cax = divider.append_axes("right", size="5%", pad=0.05)
ax.set_ylabel('$\Delta \delta$ (arcsec)')
ax.set_xlabel(r'$\Delta \alpha$ (arcsec)')
...
ax1 = plt.subplot(122)
masked_array = np.ma.array(sig_broad,mask=np.isnan(sig_broad))
im = plt.imshow(masked_array,cmap='jet',origin='lower',extent=[-6.25,6.25,-5.65,5.65])
divider = make_axes_locatable(ax1)
cax1 = divider.append_axes("right", size="5%", pad=0.05)
ax1.set_xlabel(r'$\Delta \alpha$ (arcsec)')
...
plt.savefig(path+'maps_broadH_'+str(cubo)+'.png',bbox_inches='tight',pad_inches=0.2)
