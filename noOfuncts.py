...
# Function to create the gaussian and the linear fit for Halpha+NII
def func3lines(x,slop,intc,mu_3,sig_3,amp_3,mu_2,sig_2,amp_2,mu_4,sig_4,amp_4):
    '''
    Function to fit 2 lines to a gaussian + linear.
    The parameters to introduce have to be the initial guesses. 
    x - values for the fit
    ...
    y = np.zeros_like(x)
    y = y + (slop*x+intc)
    y = y + gaussian(x,mu_2,sig_2,amp_2) + gaussian(x,mu_3,sig_3,amp_3) + gaussian(x,mu_4,sig_4,amp_4)
    return y

# Function to create the gaussian and the linear fit for Halpha+NII
def func3lines2comp(x,slop,intc,mu_3,sig_3,amp_3,mu_2,sig_2,amp_2,mu_4,sig_4,amp_4,mu_23,sig_23,amp_23,mu_22,sig_22,amp_22,mu_24,sig_24,amp_24):
    '''
    Function to fit 2 lines to a gaussian + linear.
    The parameters to introduce have to be the initial guesses. 
    x - values for the fit
    ...
    y = np.zeros_like(x)
    y = y + (slop*x+intc)
    y = y + gaussian(x,mu_2,sig_2,amp_2) + gaussian(x,mu_3,sig_3,amp_3) + gaussian(x,mu_4,sig_4,amp_4) + gaussian(x,mu_22,sig_22,amp_22) + gaussian(x,mu_23,sig_23,amp_23) + gaussian(x,mu_24,sig_24,amp_24)
    return y
